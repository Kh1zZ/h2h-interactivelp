import { Release } from '@/types';
import { Language } from '@/context/LanguageContext';

export const DISCOGRAPHY_DATA_EN: Release[] = [
  {
    id: 'the-chase',
    title: 'The Chase',
    type: 'Debut Single Album',
    releaseDate: '24 February 2025',
    description: 'The monumental debut single album by Hearts2Hearts introducing their vibrant signature sound to the world.',
    tracks: ['The Chase (Title)', 'Butterflies', 'The Chase (Inst.)'],
    coverAssetKey: 'release-cover-the-chase',
    streamingHint: 'Available on all major streaming platforms',
    audioSrc: '/assets/audio/discography/the-chase.aac',
  },
  {
    id: 'style',
    title: 'STYLE',
    type: 'Digital Single',
    releaseDate: '18 June 2025',
    description: 'A confident, catchy anthem showing off the group’s energetic charm and distinctive modern colors.',
    tracks: ['STYLE', 'STYLE (Inst.)'],
    coverAssetKey: 'release-cover-style',
    streamingHint: 'Digital Streaming',
    audioSrc: '/assets/audio/discography/style.aac',
  },
  {
    id: 'focus',
    title: 'FOCUS',
    type: '1st Mini Album',
    releaseDate: '20 October 2025',
    description: 'Rhythmic, focused, and irresistibly danceable, drawing listeners deep into the Hearts2Hearts orbit.',
    tracks: ['FOCUS', 'Apple Pie', 'Pretty Please', 'Flutter', 'Blue Moon', 'STYLE'],
    coverAssetKey: 'release-cover-focus',
    streamingHint: 'Digital Streaming & Physical EP',
    audioSrc: '/assets/audio/discography/focus.aac',
  },
  {
    id: 'rude',
    title: 'RUDE!',
    type: 'Digital Single',
    releaseDate: '20 February 2026',
    description: 'A dynamic house-based dance anthem radiating unapologetic confidence and bold energy, featuring an infectious bassline, razor-sharp hooks, and viral choreography.',
    tracks: ['RUDE!', 'RUDE! (Extended Club Mix)', 'RUDE! (Inst.)'],
    coverAssetKey: 'release-cover-rude',
    streamingHint: 'Official Digital Single • Global Streaming',
    audioSrc: '/assets/audio/discography/rude.aac',
  },
  {
    id: 'lemon-tang',
    title: 'Lemon Tang',
    type: '2nd Mini Album',
    releaseDate: '22 June 2026',
    description: 'A zesty, refreshing burst of bright pop sweetness with lively hooks and effervescent vocals.',
    tracks: [
      'Lemon Tang',
      '15-LOVE',
      '처음투성이 (Baby steps)',
      'heart emoji (♡)',
      'Secret Recipe',
      'RUDE!',
    ],
    coverAssetKey: 'release-cover-lemon-tang',
    streamingHint: 'Digital Streaming & Physical Mini Album',
    audioSrc: '/assets/audio/discography/lemon-tang.aac',
  },
  {
    id: 'iconic-heart',
    title: 'ICONIC HEART',
    type: 'Japan Debut Single Album',
    releaseDate: '12 August 2026',
    description: "Hearts2Hearts' monumental debut into the Japanese music market, delivering a blend of sweet J-Pop melodies, cheerful bilingual vocals, and captivating positive energy.",
    tracks: ['ICONIC HEART (Title)', 'Lemon Tang (Japanese Ver.)', 'RUDE! (Japanese Ver.)', 'ICONIC HEART (Inst.)'],
    coverAssetKey: 'release-cover-iconic-heart',
    streamingHint: 'Official Japan Debut • Oricon & Global Streaming',
    audioSrc: '/assets/audio/discography/iconic-heart.aac',
  },
];

export const DISCOGRAPHY_DATA_ID: Release[] = [
  {
    id: 'the-chase',
    title: 'The Chase',
    type: 'Album Single Debut',
    releaseDate: '24 Februari 2025',
    description: 'Album single debut monumental oleh Hearts2Hearts yang memperkenalkan warna musik khas mereka yang penuh semangat ke hadapan dunia.',
    tracks: ['The Chase (Title)', 'Butterflies', 'The Chase (Inst.)'],
    coverAssetKey: 'release-cover-the-chase',
    streamingHint: 'Tersedia di semua platform streaming utama',
    audioSrc: '/assets/audio/discography/the-chase.aac',
  },
  {
    id: 'style',
    title: 'STYLE',
    type: 'Single Digital',
    releaseDate: '18 Juni 2025',
    description: 'Lagu penuh percaya diri yang adiktif, menampilkan pesona energik dan warna modern yang khas dari Hearts2Hearts.',
    tracks: ['STYLE', 'STYLE (Inst.)'],
    coverAssetKey: 'release-cover-style',
    streamingHint: 'Streaming Digital',
    audioSrc: '/assets/audio/discography/style.aac',
  },
  {
    id: 'focus',
    title: 'FOCUS',
    type: 'Mini Album ke-1',
    releaseDate: '20 Oktober 2025',
    description: 'Berirama dinamis, fokus, dan memikat untuk berdansa, menarik pendengar semakin dalam ke orbit Hearts2Hearts.',
    tracks: ['FOCUS', 'Apple Pie', 'Pretty Please', 'Flutter', 'Blue Moon', 'STYLE'],
    coverAssetKey: 'release-cover-focus',
    streamingHint: 'Streaming Digital & EP Fisik',
    audioSrc: '/assets/audio/discography/focus.aac',
  },
  {
    id: 'rude',
    title: 'RUDE!',
    type: 'Single Digital',
    releaseDate: '20 Februari 2026',
    description: 'Lagu dansa berirama house yang dinamis dan memancarkan rasa percaya diri tanpa ragu, dipadukan dengan alunan bassline adiktif, hook tajam, serta koreografi viral.',
    tracks: ['RUDE!', 'RUDE! (Extended Club Mix)', 'RUDE! (Inst.)'],
    coverAssetKey: 'release-cover-rude',
    streamingHint: 'Single Digital Resmi • Streaming Global',
    audioSrc: '/assets/audio/discography/rude.aac',
  },
  {
    id: 'lemon-tang',
    title: 'Lemon Tang',
    type: 'Mini Album ke-2',
    releaseDate: '22 Juni 2026',
    description: 'Ledakan kesegaran pop manis yang ceria dengan melodi memikat dan vokal berseri bak buah lemon di musim panas.',
    tracks: [
      'Lemon Tang',
      '15-LOVE',
      '처음투성이 (Baby steps)',
      'heart emoji (♡)',
      'Secret Recipe',
      'RUDE!',
    ],
    coverAssetKey: 'release-cover-lemon-tang',
    streamingHint: 'Streaming Digital & Mini Album Fisik',
    audioSrc: '/assets/audio/discography/lemon-tang.aac',
  },
  {
    id: 'iconic-heart',
    title: 'ICONIC HEART',
    type: 'Album Single Debut Jepang',
    releaseDate: '12 Agustus 2026',
    description: 'Debut monumental Hearts2Hearts di pasar musik Jepang, menghadirkan paduan melodi manis J-Pop, vokal dwibahasa yang ceria, dan energi positif yang menawan.',
    tracks: ['ICONIC HEART (Title)', 'Lemon Tang (Japanese Ver.)', 'RUDE! (Japanese Ver.)', 'ICONIC HEART (Inst.)'],
    coverAssetKey: 'release-cover-iconic-heart',
    streamingHint: 'Debut Resmi Jepang • Oricon & Streaming Global',
    audioSrc: '/assets/audio/discography/iconic-heart.aac',
  },
];

export const DISCOGRAPHY_DATA = DISCOGRAPHY_DATA_EN;

export const getDiscographyData = (lang: Language = 'en'): Release[] => {
  return lang === 'id' ? DISCOGRAPHY_DATA_ID : DISCOGRAPHY_DATA_EN;
};
